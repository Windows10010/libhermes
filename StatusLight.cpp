...
StatusLight::StatusLight(uint8_t index)
    : Adafruit_NeoPixel(NUMBER_PIXELS, index, NEO_GRB + NEO_KHZ800),
      _blinkingState(false), _blinkingInterval(DEFAULT_BLINKING_INTERVAL) {
...
void StatusLight::setLed(int index, ledStatus status) {
  if (index > 10 || index < 0)
    return;
  if (xSemaphoreTake(this->_lock, (TickType_t)(200 / portTICK_PERIOD_MS)) ==
      pdTRUE) {
...
int StatusLight::getBlinkingInterval() { return this->_blinkingInterval; }
...
void startStatusLightController() {
  xTaskCreate(tStatusLightController,
              (const *portCHAR) "StatusLight controller", 128, NULL, 1, NULL);
}
