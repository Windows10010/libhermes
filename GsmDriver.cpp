...
template <typename T>
GsmDriver<T>::GsmDriver(T &ss)
    : AtGenericDriver<T>(ss), _phoneNumber(String(DEFAULT_SMS_RECIPIENT)) {}

template <typename T> bool GsmDriver<T>::init() {
  this->_serial.print("\n");
...
template <typename T> String GsmDriver<T>::sendSms(String messege) {
  ...
  while (true) {
    ...
    if (((unsigned long)(xTaskGetTickCount() - start)) * portTICK_PERIOD_MS >
        3000) {
